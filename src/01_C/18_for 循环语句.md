...
通过前面的学习了解了如何使用分支语句来根据不同的条件执行不同的代码，接着来看第二种重要的流程控制语句，循环语句。
...
int main() {
    printf("伞兵一号卢本伟准备就绪！");   //把这句话打印三遍
    printf("伞兵一号卢本伟准备就绪！");
    printf("伞兵一号卢本伟准备就绪！");
}
```

遇到这种情况，可以使用for循环语句来多次执行：
...
这样，利用循环，就可以批量执行各种操作了。

注意，如果表达式2什么都不写，那么会默认判定为真：
...
当然，也可以在循环过程中提前终止或是加速循环的进行，这里需要认识两个新的关键字：
...
可以看到，continue仅仅加速的是内层循环，而对外层循环没有任何效果（就近原则 ），同样的，break也只会终结离它最近的：
